...
class App extends Component{
	...
	qsstiKanvaDown(event){		
		...
		console.log('CORDZ ', finTrssm.style.height , event.pageY)
		var canvas = document.createElement('canvas');
		canvas.setAttribute('id','crp'+this.nbCrop.toString())
		canvas.height = document.getElementById('central').height
		canvas.width = document.getElementById('central').width
		...
		console.log('vvvvvv ',canvas.width,canvas.height)
		this.isCrop = 1
	}
	...
	qsstiKanvaMove(event){
		if (this.isCrop == 1) {
			...
			var canvas = document.querySelector('#crp'+(this.nbCrop-1).toString());
			console.log('vvvvvv ',canvas.width,canvas.height)
	...
	Kanva(currInstru){
		return(
				...
				<div id="central" className="row border">
					...
					<canvas id="crp1" style={{width: '100%' , height: '100%'}} onMouseDown={this.qsstiKanvaDown} onMouseUp={this.qsstiKanvaUp} onMouseMove={this.qsstiKanvaMove} ></canvas>
